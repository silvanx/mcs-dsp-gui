...
SRIO CSL EXAMPLE 
================
The folders in this directory contain a example that demonstrates the usage of SRIO CSL 3.x API's.

Hardware Requirement:
------------------
To run this example, adapter board ((AMC-TO-SMA SR10 board)) is required to connect to the EVM.
...
2. Open the Srio_ext_loopback_example.pjt in the CCS .

3. Ensure that the gel file is loaded.

4. In the project build options configure the endianness to little endian.

5. Ensure that the libraries rts64plus.lib, little endian intc (if required) and csl libraries are included.
...
5. Include the libraries rts64pluse.lib and the big endian intc (if required) and csl libraries
